...
export const ProgramCardForGuide:FC<IProgramInterface> = ({program,isComplete}) => {
  ...
  const participants = () =>{
    if(isComplete){
      ...
      return(
        <>
          <div style={{border:"0.5px solid gray",color:COLOR.primary,textAlign:"right"}}>{program.num_participant}/{program.max_participant}</div>
          </>
      );
    }
  }
  ...
  return( 
    ...
    {/* <Link key={program._id} href={`/trips/programDetail/${program._id}`}> */}
      <div key={program._id}    onClick={handleClick} style={{display:"flex", flexDirection:"column",border:"1px solid black" }}>
      <div>
